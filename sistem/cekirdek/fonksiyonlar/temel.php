...
function m_jsonAl($json = "", $al = "", $yaz = null) {
    if (!is_object($json) && !is_array($json)) {$json = json_decode($json);}
    if (is_object($json)) {
        if (isset($json->$al)) {return $json->$al;}
...
function m_jsonDegistir($json = "", $bul = "", $yaz = null, $r_obj = 0) {
    ...
    @$json->{$bul} = $yaz;
    if ($r_obj === true) {return $json;} elseif ($r_obj === false) {return $json = json_encode($json);} elseif ($is_json) {return $json;} else {return $json = json_encode($json);}
}
...
function m_ekyaz($ek = null, $v = 'icerik', $h = true) {
    ...
    if ($ek != null) {
        ...
        if (isset($m_vt)) {
            ...
            if (count($al) == 1) {
                if ($v === true) {return $al[0];} elseif (isset($al[0]->{$v})) {$r = $al[0]->{$v};}
            }
        }
    }
    return $r;
...
function m_sayfano($l = 1, $c = 0, $otoget = true, $getSkip = null, $urls1 = null, $urls2 = null) {
    if (is_numeric($l) && $l > 0 && is_numeric($c)) {
        if ($c > $l) {$ss = ceil($c / $l);} else { $ss = 1;}
        if (isset($_GET['p']) && $_GET['p'] > 0) {$s = intval($_GET['p']);} else { $s = 0;}
        if ($s >= $ss) {
            $s = $ss - 1;
        ...
        if ($c > 0) {
            ...
            if ($is < 1 || $is == 2) {$is = 1;}
            if ($rs >= $ts - 1) {$rs = $ts;}
            if ($s > $f + 2) {$r .= "<li><a href='" . m_otoGets("p", 1, $getSkip) . "'>&nbsp;1 &laquo;&nbsp;</a></li>";}
            for ($i = $is; $i <= $rs; $i++) {
                if ($s == $i) {$r .= "<li class='active'><span>$i</span></li>";} else if (!$otoget) {$r .= "<li><a href='" . $urls1 . $i . $urls2 . "'>$i</a></li>";} else { $r .= "<li><a href='" . m_otoGets("p", $i, $getSkip) . "'>$i</a></li>";}
            }
            if (!$otoget == 2 && $rs < ($ts - $f + 2)) {$r .= "<li><a href='" . $urls1 . $ts . $urls2 . "'>&nbsp;&raquo $ts&nbsp;</a></li>";} else if ($rs < ($ts - $f + 2)) {$r .= "<li><a href='" . m_otoGets("p", $ts, $getSkip) . "'>&nbsp;&raquo $ts&nbsp;</a></li>";}
            return $r . "</ul></nav>";
        }
    }
}
...
function m_otoGets($getKey = null, $getVal = null, $getSkip = null) {
    if (!empty($getKey)) {
        if (count($_GET)) {
            ...
            $urlReturn = null;
            $nonThisGet = true;
            if (!empty($getSkip)) {foreach (explode(",", $getSkip) as $value) {if (isset($_GET[$value])) {unset($_GET[$value]);}}}
            foreach ($_GET as $key => $value) {
                if ($getKey == $key) {
                    $urlReturn = $urlReturn . "&" . $key . "=" . $getVal;
                    $nonThisGet = false;
                } else {
                    if (is_array($value)) {foreach ($value as $key1 => $value1) {$urlReturn = $urlReturn . "&" . $key . "%5B%5D=" . $value1;}
                    } else { $urlReturn = $urlReturn . "&" . $key . "=" . $value;}
                }
            }
            if ($nonThisGet) {$urlReturn = $urlReturn . "&" . $getKey . "=" . $getVal;}
            $urlReturn = $m_url . "?" . ltrim($urlReturn, "&");
        } else { $urlReturn = m_url . "?" . $getKey . "=" . $getVal;}
    } else { $urlReturn = $_SERVER['REQUEST_URI'];}
    return $urlReturn;
}
...
function m_url_duzelt($txt = null) {
    ...
    $txt = str_replace("-", " ", $txt);
    $kelime = explode(" ", $txt);
    foreach ($kelime as $deger) {if ($deger != "") {$txt2 .= $deger . " ";}}
    $txt = $txt2;
    $txt = trim($txt);
    $txt = str_replace(" ", "-", $txt);
    $txt = trim($txt);
...
function m_all_up($metin){
    return utf8_encode(strtoupper(utf8_decode($metin)));
}

function m_all_low($metin){ 
    return mb_convert_case(str_replace('I','ı',$metin), MB_CASE_LOWER, "UTF-8"); 
}

function m_ucword($metin) { 
