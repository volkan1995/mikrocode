...
class m_baglan{
    ...
    // Manuel Sorgu
    public function sorgu($sql = null, $val = array(), $obj = "FETCH_OBJ"){
        ...
        if($sorgu && $sorgu->rowCount()>0){
            ...
            switch ($obj) {
                ...
                case "FETCH_COLUMN":
                    ...
                    break;
                case "FETCH_NUM":
                    return $sorgu->fetchAll(PDO::FETCH_NUM);
                    break;
                case "FETCH_BOUND":
                    return $sorgu->fetchAll(PDO::FETCH_BOUND);
                    break;  
                case "FETCH_BOTH":
    ...
    public function sortby($sorgu = array()){
        ...
        if(is_array($sorgu) && isset($sorgu['table']) && isset($sorgu['sort'])){
            ...
            if(count($jsn)){
                ...
                if(!isset($sorgu['group'])){ $sorgu['group'] = null; }else{ $sorgu['group'] = "GROUP BY `" . str_replace(["`","'",'"','.'],"",$sorgu['group']) . "`"; }
                if(!$sorgu_prnt){
                    $sql = "SELECT `{$sorgu['id']}`,`{$sorgu['order']}` FROM `{$sorgu['table']}`{$sorgu['group']}";
                }else{
                    $sql = "SELECT `{$sorgu['id']}`,`{$sorgu['parent']}`,`{$sorgu['order']}` FROM `{$sorgu['table']}`{$sorgu['group']}";
                }
