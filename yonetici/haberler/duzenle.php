                ...
                <div class="card">
                    ...
                    <div class="body">
                        ...
                        <div class="tab-content">
                            <?php
                            foreach ($sorgu_icerikler as $dil => $icerik) {
                                $icerik->eklenti = json_decode($icerik->eklenti);
                                ?>
                                <fieldset role="tabpanel" data-dil="<?= $dil ?>" class="row tab-pane fade" id="form_<?=$dil?>">
                                    <div class="col-md-9 col-sm-8 col-xs-12">
                                        <div class="form-group">
                                            <div class="form-line">
                                                <input type="text" class="form-control" name="baslik" placeholder="<?= mc_dil('baslik_giriniz') ?>" value="<?= $icerik->baslik ?>" required/>
                                            </div>
                                        </div>
                                        <?php if (isset($mc_modul_ayar['tasarim']['kategori']) && $mc_modul_ayar['tasarim']['kategori']) { ?>
                                            <div class="m-t-25">
                                                <select name="kategori" class="form-control show-tick"><?= mc_kategoriler(['dil' => $dil, 'tip' => $mc_modul_ayar['kategori'], 'sec' => $icerik->kategori]) ?></select>
                                            </div> 
                                        <?php } if (isset($mc_modul_ayar['tasarim']['aciklama']) && $mc_modul_ayar['tasarim']['aciklama']) { ?>
                                            <div class="form-group m-t-20">
                                                <div class="form-line">
                                                    <input type="text" class="form-control" name="aciklama" placeholder="<?= mc_dil('aciklama_giriniz') ?>" value="<?= $icerik->aciklama ?>"/>
                                                </div>
                                            </div> 
                                            <?php
                                        }                                        
                                        if (isset($mc_modul_ayar['tasarim']['icerik']) && $mc_modul_ayar['tasarim']['icerik']) {
                                            echo '<div class="m-t-30">' . mc_editor($icerik->icerik) . '</div>';
                                        }
                                        if (isset($mc_modul_ayar['tasarim']['galeri']) && $mc_modul_ayar['tasarim']['galeri']) {
                                            echo '<div class="m-t-30">' . mc_dosyalar("gorsel", m_jsonAl($icerik->eklenti, "galeri", []), "galeri", ['baslik' => "Galeri Resimleri Seç / Yükle"]) . '</div>';
                                        }
                                        if (isset($mc_modul_ayar['tasarim']['ek']) && $mc_modul_ayar['tasarim']['ek']) {
                                            echo '<div class="m-t-30">' . mc_dosyalar("dosya", m_jsonAl($icerik->eklenti, "ek", []), "ek", ['baslik' => "Ek Seç / Yükle", 'detaylar' => true]) . '</div>';
                                        } 
                                        ?>
                                    </div>
                                    <div class="col-md-3 col-sm-4 col-xs-12">
                                        <input type="hidden" name="id" data-skipcopy="true" value="<?= $icerik->id ?>" />
                                        <div class="switch m-t-20">
                                            <b class="mc_sw_b80"><?= mc_dil('yayin') ?></b>
                                            <label><input name="yayin" type="checkbox"<?= $icerik->durum == 1 ? " checked" : null ?>/><span class="lever switch-col-theme"></span></label>
                                        </div>
                                        <?php if (isset($mc_modul_ayar['tasarim']['vurgula']) && $mc_modul_ayar['tasarim']['vurgula']) { ?>
                                        <div class="switch m-t-20">
                                            <b class="mc_sw_b80"><?= mc_dil('manset') ?></b>
                                            <label><input name="vurgula" type="checkbox"<?= $icerik->vurgula == 1 ? " checked" : null ?>/><span class="lever switch-col-theme"></span></label>
                                        </div>
                                        <?php
                                        }
                                        if (isset($mc_modul_ayar['tasarim']['resim']) && $mc_modul_ayar['tasarim']['resim']) {
                                            echo '<div class="m-t-10">' . mc_dosya("gorsel", $icerik->resim, "kapak") . '</div>';
                                        }
                                        if (isset($mc_modul_ayar['tasarim']['video']) && $mc_modul_ayar['tasarim']['video']) {
                                            echo '<div class="m-t-10">' . mc_dosya("video", m_jsonAl($icerik->eklenti, 'video', 0), "video") . '</div>';
                                        }
                                        ?>
                                    </div>  
                                </fieldset>
