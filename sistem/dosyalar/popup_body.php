<?php if(!defined('m_guvenlik')) { exit; } ?>
    <?php if(empty($_POST['tip']) && (!isset($_POST['klasor']) || empty($_POST['klasor']))){ $mc_anadizinler = true; foreach ($tip_dizi as $dizinno => $anadizin) { if($dizinno == 0){ continue; } ?>
        <div class="mc_klasor_list">
            <a href="#" target="dir" class="oge col-blue-grey" data-id="0" data-isim="<?=$anadizin?>" data-tip="<?=$dizinno?>">
    ...
    <?php } }else{ $mc_anadizinler = false; } foreach ($klasor_tara as $klasor) { ?>
        <div class="mc_klasor_list">
            <a href="#" target="dir" class="oge col-blue-grey" data-id="<?=$klasor->id?>" data-isim="<?=$klasor->baslik?>" data-tip="<?=$_POST['tip']?>">
                <div class="dosya_simge"><i class="material-icons"><?php
                        if($_POST['tip'] == 1){
                            echo 'photo_library';
                        }elseif($_POST['tip'] == 2){
    ...
    <?php }  foreach ($dosya_tara as $dosya) { ?>
        <div class="mc_klasor_list mc_liste_cbx" id="mc_liste_<?=$dosya->id?>">
