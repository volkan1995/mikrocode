    ...
    foreach ($_POST['id'] as $dil => $id) {
        ...
        if ($id != 0) {
            ...
            if (count($varlik_sor) == 0) {
                mc_uyari(-3, "<b>" . mc_dil('basarisiz') . "</b>Kategori kaydı bulunamadı $dil : " . $_POST['baslik'][$dil]);
            }
            mc_ic_yetki($varlik_sor[0]->izinler, 1, true);
            if ($dil_grup == 0) {
        ...
        if ($id == 0) {
            if (count($m_vt->select()->from($mc_modul_ayar['tablo'])
                                    ->where("sef", $_POST['sef'][$dil])->where("ust", $_POST['kategori'][$dil])->where("tip", $mc_modul_ayar['tip'])
                                    ->result()) > 0) {
                mc_uyari(-3, "<b>" . mc_dil('basarisiz') . "</b>Zaten benzer bir kategori kaydı mevcut. $dil : " . $_POST['baslik'][$dil]);
            }
        } else {
            if (count($m_vt->select()->from($mc_modul_ayar['tablo'])
                                    ->in()->where("sef", $_POST['sef'][$dil])->where("ust", $_POST['kategori'][$dil])->where("tip", $mc_modul_ayar['tip'])
                                    ->out()->where("id", $id, "<>")
                                    ->result()) > 0) {
                mc_uyari(-3, "<b>" . mc_dil('basarisiz') . "</b>Zaten benzer bir kategori kaydı mevcut. $dil : " . $_POST['baslik'][$dil]);
            }
...
$get_kategori = $m_vt->select()->from($mc_modul_ayar['tablo'])->where("id", $_GET['id'])->result();
if (!count($get_kategori)) {
    exit;
}
$mc_title = mc_dil('duzenle') . ": " . $get_kategori[0]->baslik;

$get_kategoriler = array();
$sorgu_kategoriler = $m_vt->select()->from($mc_modul_ayar['tablo'])->where("grup", $get_kategori[0]->grup)->orderby('id', "ASC")->result();
foreach ($sorgu_kategoriler as $sorgu_kategori) {
    $get_kategoriler[$sorgu_kategori->dil] = $sorgu_kategori;
}
