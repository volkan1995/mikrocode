<?php
    ...
    function m_jsonDegistir($json="", $bul = "", $yaz = null, $r_obj = 0){
        ...
        @$json->{$bul} = $yaz;
        if($r_obj === true){ return $json; }
        elseif($r_obj === false){ return $json = json_encode($json); }
        elseif($is_json){ return $json; }
        else{ return $json = json_encode($json); }
    }
    ...
    function m_ekyaz($ek = null, $v = 'icerik', $h = true){
        ...
        if($ek != null){
            ...
            if(isset($m_vt)){
                ...
                if(count($al) == 1){
                    if($v === true){ return $al[0]; }
                    elseif(isset($al[0]->{$v})){ $r = $al[0]->{$v}; }
                }
            }
        }
        return $r;
    ...
    function m_sayfano($l = 1, $c = 0, $otoget=true, $getSkip=null, $urls1=null, $urls2=null){
        if(is_numeric($l) && $l > 0 && is_numeric($c)){         
            ...
            if($c > 0){
                ...
                for($i=$is;$i<=$rs;$i++){
                    if($s==$i){$r.="<li class='active'><span>$i</span></li>"; }
                    else if(!$otoget){ $r.="<li><a href='".$urls1.$i.$urls2."'>$i</a></li>"; }
                    else{ $r.="<li><a href='".m_otoGets("p",$i,$getSkip)."'>$i</a></li>"; }
                }
                if(!$otoget==2 && $rs < ($ts - $f + 2)){ $r.="<li><a href='".$urls1.$ts.$urls2."'>&nbsp;&raquo $ts&nbsp;</a></li>"; }
                else if($rs < ($ts - $f + 2)){ $r.="<li><a href='".m_otoGets("p",$ts,$getSkip)."'>&nbsp;&raquo $ts&nbsp;</a></li>"; }
                return $r."</ul></nav>";
            }
        }
    }
