...
if (isset($_POST['kaydet'])) {
    ...
    foreach ($_POST['diller'] as $dil) {
        ...
        if ($m_vt->select()->from($mc_modul_ayar['tablo'])
                        ->where("sef", $_POST['sef'][$dil])
                        ->where("ust", $_POST['kategori'][$dil])
                        ->where("tip", $mc_modul_ayar['tip'])
                        ->count() > 0) {
            mc_uyari(-3, "<b>" . mc_dil('basarisiz') . "</b>Zaten benzer bir kategori kaydı mevcut. " . $_POST['baslik'][$dil]);
        }
    ...
    if ($mc_kaydet) {
        mc_uyari(-1, "<b>" . mc_dil('basarili') . "</b>Kategori eklendi");
    } else {
        mc_uyari(-4, "<b>" . mc_dil('basarili') . "</b>Kategori bilinmeyen bir hatadan dolayı eklenemedi.");
    }
}

$mc_title = mc_dil($mc_modul_ayar['ekle']);

$mc_kategoriler = $m_vt->select();
