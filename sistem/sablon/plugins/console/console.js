(function (root, factory) {
	...
	var containerHtml = '' + 
	'<div id="jsmc-collapse" class="btn btn-default btn-xs waves-effect"></div>' +
	'<div id="jsmc-clear" class="btn btn-danger btn-xs waves-effect">&#xd7 Temizle</div>' +
	'<div id="jsmc-button" class="btn btn-default btn-xs waves-effect">&#9654; Çalıştır</div>' +
	'<div id="jsmc-commands-container"></div>' +	
	'<div id="jsmc-content" class="form-group m-t-10">' +
	'	<div id="jsmc-input-container" class="form-line">' +
	'		<input id="jsmc-input" type="text" class="form-control" placeholder="Javascript kodlarını buraya yazabilirsiniz"/>' +
	'	</div>' +
	'</div>' +
	'<div id="jsmc-log">' +
	'	</div>';
	...
	var mobileConsole = {
		...
		initializeContainers: function(options){
			...
			document.getElementById('mc_console').appendChild(el);
			...
			if (this.props.isCollapsed){
				...
				this.$el.collapseControl.innerHTML = '&#9660; Göster';
			} else {
				this.$el.collapseControl.innerHTML = '&#9650; Gizle';				
			}
		},
		...
		toggleCollapsed: function(toBeCollapsed){
			...
			this.$el.collapseControl.innerHTML = this.isCollapsed ? '&#9660; Göster' : '&#9650; Gizle';
			if (this.isCollapsed){
				this.$el.clearControl.style.display = 'none';
			} else {
