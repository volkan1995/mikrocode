...
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', '../../depo/onbellek/resimler');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
class timthumb {
	...
	public function __construct(){
		...
		$this->src = preg_replace('/https?:\/\/(?:www\.)?' . $this->myHost . '/i', '', $this->src);
                if($this->hotLink){
                    $this_srcUrl = parse_url($this->src);
                    if(isset($this_srcUrl['host'])){
                        $this_srcUrl = strtolower(implode('.', array_slice(explode('.', $this_srcUrl['host']), -2, 2)));
                        $this_hostUrl = strtolower(implode('.', array_slice(explode('.', $this->myHost), -2, 2)));
                        if($this_srcUrl != $this_hostUrl){
                            $this->error("Resim farklı sunucudan istendi");
                            return false;
                        }                    
                    }
                }
		if(strlen($this->src) <= 3){
			$this->error("Resim istenmedi");
			return false;
		}
		...
		if($this->isURL){
			...
			if(! $this->localImage){
				$this->debug(1, "Konum: {$this->localImage}");
				$this->error("Resim bulunamadı.");
				$this->set404();
				return false;
			}
	...
	protected function serveErrors(){
		...
		$html .= '</ul>';
		echo '<div style="margin: 40 auto;display:table;max-width: 400px;border: 3px dashed #bbb;padding: 20px;">';
                echo '<div style="float:left; width:100%; text-align:center;"><img src="/mc-yonetici/resimler/logo1.png" style="width:300px;">';
                echo '<h1>Resim gösterilemiyor<br/>Neden(ler)</h1>' . $html . '</div>';
                if(isset($_GET) && count($_GET)){
                    $mcgetargs['src'] = "Adres";
                    $mcgetargs['w'] = "Genişlik";
                    $mcgetargs['h'] = "Yükseklik";
                    $mcgetargs['zc'] = "Boyutlandırma";
                    echo '<div style="float:left; width:100%; margin-top:30px;">';
                    foreach ($_GET as $key => $value) {
                        if(isset($mcgetargs[strtolower($key)])){
                            $key = $mcgetargs[strtolower($key)];
                        }
